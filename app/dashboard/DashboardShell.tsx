...
export default function DashboardShell({
 ...
 const navItems: { key: 'all' | 'active' | 'completed'; label: string; count: number; icon: React.ReactNode }[] = [
    { key: 'all', label: 'All portals', count: portals.length, icon: <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" /> },
    { key: 'active', label: 'Active', count: activePortals.length, icon: <path strokeLinecap="round" strokeLinejoin="round" d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" /> },
    { key: 'completed', label: 'Completed', count: completedPortals.length, icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /> },
  ]
 ...
 className={`flex items-center gap-3 px-2.5 py-2.5 rounded-lg text-sm overflow-hidden ${filter === item.key ? 'bg-[#16161a] text-white' : 'text-gray-500 hover:text-gray-300 hover:bg-[#101013]'}`}>
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-10 py-9">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-9">
            {[
              ...
              const toneCls = s.tone === 'green'
                ? 'bg-green-400/10 text-green-400'
                : s.tone === 'brand'
                ? 'bg-[#8b3cf7]/10 text-[#8b3cf7]'
                    ...
                    <a key={portal.id} href={`/dashboard/portal/${portal.id}`}
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 border ${paid ? 'bg-green-400/5 border-green-400/20' : 'bg-[#1a0d30] border-[#8b3cf7]/20'}`}>
                          {paid ? (
                            <svg className="w-4.5 h-4.5 text-green-400" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                          ) : (
                            <svg className="w-4.5 h-4.5 text-[#8b3cf7]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" /></svg>
                          )}
                        </div>
                        <div className="min-w-0">
                      ...
                      {/* Views — desktop column / mobile hidden */}
                      <div className="hidden md:flex items-center gap-1.5 text-xs text-gray-500">
                        ...
                        {v ? (
                          <span className="truncate">{v.count}{v.lastViewed && <span className="text-gray-600"> · {timeAgo(v.lastViewed)}</span>}</span>
                        ) : <span className="text-gray-600">—</span>}
                      </div>

                      {/* Amount — desktop column */}
                      ...
                      {/* Status + chevron (right side on both layouts) */}
