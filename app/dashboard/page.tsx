...
export default async function DashboardPage() {
  ...
  const storageLimit = storageLimits[planKey] || storageLimits.free
  ...
  const limitLabel = planKey === 'free' ? '1 GB' : planKey === 'pro' ? '25 GB' : '250 GB'
    ...
    <main className="min-h-screen bg-[#090909] text-white">
      <nav className="border-b border-[#1e1e24] px-8 py-4 flex items-center justify-between bg-[#090909]/80 backdrop-blur-sm sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="/settings" className="text-gray-400 hover:text-white border border-[#1e1e24] hover:border-[#3a3a4a] p-1.5 rounded-lg transition-colors flex items-center justify-center" title="Settings">
      ...
      <div className="max-w-5xl mx-auto px-8 py-10">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${
              planKey === 'agency' ? 'bg-yellow-400/10 text-yellow-400 border-yellow-400/20' :
              planKey === 'pro' ? 'bg-[#8b3cf7]/10 text-[#8b3cf7] border-[#8b3cf7]/20' :
              'bg-[#1e1e24] text-gray-500 border-[#1e1e24]'
            }`}>
              {planKey === 'agency' ? 'Agency' : planKey === 'pro' ? 'Pro' : 'Free plan'}
            </span>
          </div>
        </div>
        ...
        {/* Storage usage */}
        <div className="bg-[#111114] border border-[#1e1e24] rounded-xl p-5 mb-8">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold">Storage</p>
            <div className="flex items-center gap-3">
              <p className="text-xs text-gray-400">{formatBytes(usedBytes)} / {limitLabel}</p>
              {planKey === 'free' && (
                <a href="/pricing" className="text-xs text-[#8b3cf7] hover:underline font-semibold">Upgrade</a>
              )}
            </div>
          </div>
          <div className="w-full bg-[#1e1e24] rounded-full h-1.5">
            <div
              className={`h-1.5 rounded-full transition-all ${storagePercent >= 90 ? 'bg-red-400' : storagePercent >= 70 ? 'bg-yellow-400' : 'bg-[#8b3cf7]'}`}
          ...
          {storagePercent >= 90 && (
            <p className="text-xs text-red-400 mt-2">
              Storage almost full —{' '}
              <a href="/pricing" className="underline font-semibold">upgrade your plan</a>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {planKey === 'free' && portals && portals.length >= 3 && (
              <a href="/pricing" className="text-xs text-[#8b3cf7] hover:underline font-semibold">
